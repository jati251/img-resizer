...
import { X, Download, FileJson, FileImage, Settings, HardDrive } from "lucide-react";
...
export const ExportModal: React.FC<ExportModalProps> = ({ canvas, filename, onClose, onExport }) => {
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
       ...
       setEstimatedSize(size > 1024 * 1024 ? `${(size / (1024 * 1024)).toFixed(2)} MB` : `${(size / 1024).toFixed(0)} KB`);
    };
      ...
      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl flex flex-col">
        <div className="p-6 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/30">
          <div className="flex items-center gap-3">
             ...
             <h2 className="text-sm font-bold text-white uppercase tracking-widest">Export Design</h2>
          </div>
        ...
        <div className="p-8 space-y-8 flex-1">
          ...
          {settings.format !== "image/png" && (
             <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
               <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-mono text-blue-400">{Math.round(settings.quality * 100)}%</span>
               </div>
               <input 
                  type="range" 
                  min={0.1} 
          ...
          <div className="p-6 bg-zinc-950/50 rounded-2xl border border-zinc-800 flex items-center justify-between">
             <div className="flex items-center gap-3">
                ...
                <div>
                   <div className="text-[10px] font-bold text-zinc-500 uppercase">Estimated Size</div>
                   <div className="text-sm font-bold text-white font-mono">{estimatedSize}</div>
                </div>
             </div>
             <div className="text-right">
                <div className="text-[10px] font-bold text-zinc-500 uppercase">Resolution</div>
                <div className="text-sm font-bold text-zinc-300 font-mono">{canvas?.width} × {canvas?.height}</div>
             </div>
          </div>
        </div>
