...
export const Topbar: React.FC<TopbarProps> = ({
  ...
  const menus = [
    ...
    },
    {
      ...
      items: [
        {
          label: showGuidelines ? "Hide Rulers" : "Show Rulers",
          icon: showGuidelines ? <EyeOff size={14} /> : <Eye size={14} />,
  ...
  return (
    <header className="h-14 border-b border-zinc-800/50 bg-zinc-900/80 backdrop-blur-xl flex items-center justify-between px-4 z-[100] shrink-0">
      <div className="flex items-center gap-6" ref={menuRef}>
        <div className="flex items-center gap-3 pr-2 border-r border-zinc-800/50 mr-2">
          ...
          <div className="flex flex-col -space-y-1 hidden sm:flex">
            <h1 className="text-sm font-black tracking-tighter text-zinc-100 uppercase italic leading-none">
              Kuwas
            </h1>
            <span className="text-[8px] text-zinc-500 font-medium uppercase tracking-[0.2em] pl-0.5">
              Studio
            </span>
          </div>
        </div>
                  ...
                  <div className="p-1.5">
                    {menu.items.map((item, idx) => (
                      ...
                      >
                        ...
                        {item.shortcut && (
                          <span className="text-[9px] text-zinc-600 group-hover:text-blue-100 font-mono">
                            {item.shortcut}
                          </span>
                        )}
                      </button>
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={onExport}
          className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-blue-500/20 transition-all active:scale-95"
        >
